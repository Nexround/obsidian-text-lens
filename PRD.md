...
**F-1 命令触发**  
注册 Obsidian 命令 `TextLens: OCR Current Note`，在命令面板（`Cmd+P`）中可调用，作用于当前活跃的 Markdown 文件。
...
- **Phase 1（并发）**：所有图片的文件读取和 OCR 调用并发执行，最大并发数由用户设置控制（默认 3，可调 1–20）。Notice 实时显示已完成数量。
...
Obsidian Plugin (Electron Renderer — app:// context)
        ...
        │       │       │
        │       │       └── runOcr(plugin, img, ...)
        │       │               └── fileToArrayBuffer(vault, path)  fetch(app://)
...
| 文件 | 职责 |
|------|------|
| `src/main.ts` | 插件入口、命令注册、设置 UI、OCR 调度（`runOcr`）、文本格式化与插入、换行合并（`refineLineBreaks`） |
...
| **v1.3.1**（当前） | 删除远程 API 模式（本地 only）、Setup 完成后设置面板立即刷新、Max concurrency 改为文本输入框（范围 1–20）、manifest description 更新 |
